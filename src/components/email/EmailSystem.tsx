...
import { FileText, Loader2, Mail, RefreshCw, Send, User } from 'lucide-react';
  ...
  const handleSaveTemplate = () => {
    if (!newTemplate.name.trim() || !newTemplate.subject.trim() || !newTemplate.content.trim()) {
      toast({
        ...
        description: "Please fill in all fields to create a new template",
        variant: "destructive"
      });
      return;
    }
    const existingTemplate = templates.find(template => template.name === newTemplate.name);
    if (existingTemplate) {
      toast({
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Select
                  value={selectedContacts.length > 0 ? selectedContacts[0] : ''}
                  onValueChange={(value) => {
                    // Toggle selection
                    if (selectedContacts.includes(value)) {
                      setSelectedContacts(selectedContacts.filter(id => id !== value));
                    } else {
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card className="hover:shadow-md transition-shadow">
              <CardHeader>
                ...
                <CardDescription>Start from scratch or duplicate existing</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Input
